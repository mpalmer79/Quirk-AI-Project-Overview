...
(function () {
  ...
  // 1) Scroll reveal (+ dramatic entrance for the countdown card)
    ...
    (entries) => {
      entries.forEach((e) => {
        if (e.isIntersecting) {
          ...
          // If it's the hero countdown card, run the fancy entrance
          if (e.target.matches('.hero .countdown-wrap') && !prefersReduce) {
  ...
  toReveal.forEach((el, i) => {
    ...
    if (!prefersReduce) {
      ...
      // Respect Reduce Motion: show immediately, skip animation
      el.classList.add('in');
    }
  });
